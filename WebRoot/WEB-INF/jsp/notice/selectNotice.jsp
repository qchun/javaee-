...
<html>
  ...
  <body>
    ...
    <form id="f1" action="selectNotice" method="post">
    	<table border="0" cellspacing="0" cellpadding="0" width="80%" height="50px">
        	<tr>
        		<td><input type="hidden" name="pageNow" id="pageNow"></td>
          		<td>公告名称：<input type="text" name="title" value="${requestScope.notice.title }"></td>
          		<td>公告内容：<input type="text" name="content" value="${requestScope.notice.content }"></td>
          		<td>公告人：
				<select name="user_id">
          			<option value="">请选择公告人</option>
          			<c:forEach items="${userList}" var="user">
          				<option value="${user.id}" <c:if test="${requestScope.notice.user_id==user.id }">selected</c:if>>${user.username }</option>
          			</c:forEach>
          		</select></td>
	...
	<table border="1" cellspacing="0" cellpadding="0" width="80%">
		<tr align="center" height="30px">
			<td>
				...
				<input type="submit" value="删除" onclick="javascript:return confirm('确认删除这些公告吗？');"/>
			</td>
			<td>公告名称</td>
			<td>公告内容</td>
			<td>创建时间</td>
		...
		<c:forEach items="${requestScope.noticeList}" var="nl">
			<tr align="center" height="30px">
				<td><input type="checkbox" name="ids" value="${nl.id }"/></td>
				<td>${nl.title }</td>
				<td><div id="d">${nl.content }</div></td>
				<td><fmt:formatDate value="${nl.create_date }" pattern="yyyy年MM月dd日 HH:mm:ss"/></td>
				<td>${nl.user.username }</td>
				<td>
					<a href="updateNoticeView?id=${nl.id}">修改</a>&nbsp;&nbsp;&nbsp;
					<a href="deleteNotice?id=${nl.id}" onclick="javascript:return a();">删除</a>&nbsp;&nbsp;&nbsp;
					<a href="ylNoticeView?id=${nl.id}">预览</a>
				</td>
			</tr>
		</c:forEach>
	</table>
